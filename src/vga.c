...
void vga_puts(Vec2_u8 p, const char *str, VgaColor fg, VgaColor bg) {
    for(u16 i=0 ; str[i] ; ++i) {
...
void vga_puts_logd(Vec2_u8 p, const char *str, VgaColor fg, VgaColor bg) {
    logd(str);
    vga_puts(p, str, fg, bg);
}
void vga_set_cursor(Vec2_u8 p) {
    u16 i = p.y*VGA_W + p.x;
